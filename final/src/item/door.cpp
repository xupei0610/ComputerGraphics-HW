...
item::Door::Door(std::size_t item_id)
        : Item(item_id),
          ...
          mesh(nullptr), is_lighting(false)
{
    ...
    light_source.coef     = glm::vec3(0.f, 0.f, 0.75f);
}
...
void item::Door::enlight()
{
    is_lighting = true;
}

bool item::Door::lighting()
{
    return is_lighting;

}

const Light &item::Door::light()
{
    return light_source;
}
...
glm::vec3 item::Door::pos()
{
    return position;
}
glm::vec3 item::Door::halfSize()
{
    return scale;
}
                        ...
                        glm::mat4 const &proj)
{
    shader->use();
    if (is_lighting)
        shader->set("material.ambient", ambient+0.5f);
    else
...
std::shared_ptr<Item> item::MetalDoor::create()
{
    return std::shared_ptr<Item>(new MetalDoor);
}
...
std::shared_ptr<Item> item::WoodDoor::create()
{
    return std::shared_ptr<Item>(new WoodDoor);
}
...
std::shared_ptr<Item> item::WaterDoor::create()
{
    return std::shared_ptr<Item>(new WaterDoor);
}
...
std::shared_ptr<Item> item::FireDoor::create()
{
    return std::shared_ptr<Item>(new FireDoor);
}
...
std::shared_ptr<Item> item::EarthDoor::create()
{
    return std::shared_ptr<Item>(new EarthDoor);
}
